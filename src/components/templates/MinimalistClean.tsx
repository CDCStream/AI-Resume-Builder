...
export default function MinimalistClean({ resume }: TemplateProps) {
  ...
  const theme: ThemeConfig = { headingClass: "text-sm font-semibold text-gray-800 uppercase tracking-wider mb-3", textClass: "text-sm text-gray-600", subTextClass: "text-xs text-gray-400", accentColor: "#374151", dotFilledClass: "bg-gray-700", dotEmptyClass: "bg-gray-200", tagClass: "text-sm text-gray-600 bg-gray-100 px-3 py-1 rounded-full" };
    ...
    <div className="resume-page w-[210mm] min-h-[297mm] bg-white p-14 shadow-lg print:shadow-none">
      ...
      <header className="mb-10 flex items-start gap-6">
        ...
        <div className="flex-1">
          ...
          {basics?.birthDate && (
            <span className="text-gray-500 text-sm">{formatDate(basics.birthDate)}</span>
          )}
