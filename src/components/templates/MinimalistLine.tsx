...
export default function MinimalistLine({ resume }: TemplateProps) {
  ...
  const theme: ThemeConfig = { headingClass: "text-xs font-semibold text-gray-500 uppercase tracking-[0.2em] mb-3 pb-1 border-b border-gray-200", textClass: "text-sm text-gray-600", subTextClass: "text-xs text-gray-400", accentColor: "#6B7280", dotFilledClass: "bg-gray-600", dotEmptyClass: "bg-gray-200", tagClass: "text-sm text-gray-600 bg-gray-100 px-3 py-1 rounded-full" };
const orderedSections = useOrderedSections(resume);


  return (
    <div className="resume-page w-[210mm] min-h-[297mm] bg-white font-['Inter',sans-serif] text-gray-900 shadow-lg" style={{ padding: '64px' }}>
      {/* Header - Centered */}
      <header className="text-center mb-12">
        ...
        {basics?.birthDate && (
          <span className="text-gray-500 text-sm mt-2">{formatDate(basics.birthDate)}</span>
        )}
