...
export default function MinimalistLine({ resume }: TemplateProps) {
  ...
  return (
    <div className="resume-page w-[210mm] min-h-[297mm] bg-white p-16 font-['Inter',sans-serif] text-gray-900 shadow-lg print:shadow-none">
          ...
          <div className="space-y-3">
            {publications.filter(p => p.name).map((pub, index) => (
              <div key={index} className="text-center">
                ...
                {pub.summary && <p className="text-sm text-gray-600 mt-1 max-w-2xl mx-auto">{pub.summary}</p>}
              </div>
            ))}
          </div>
        </section>
          ...
          <div className="space-y-3">
            {projects.filter(p => p.name).map((project, index) => (
              <div key={index} className="text-center">
                ...
                {project.description && <p className="text-sm text-gray-600 mt-1 max-w-2xl mx-auto">{project.description}</p>}
          ...
          <div className="space-y-3">
            {volunteer.filter(v => v.organization).map((vol, index) => (
              <div key={index} className="text-center">
                ...
                {vol.summary && <p className="text-sm text-gray-600 mt-1 max-w-2xl mx-auto">{vol.summary}</p>}
              </div>
            ))}
          </div>
        </section>
      ...
      {strengths && strengths.length > 0 && strengths.some(s => s.name) && (<section className="mb-10"><h2 className="text-center text-xs font-medium tracking-[0.3em] text-gray-400 mb-4">STRENGTHS</h2><div className="space-y-3">{strengths.filter(s => s.name).map((s, i) => (<div key={i} className="text-center"><h3 className="font-semibold text-gray-900">{s.name}</h3>{s.description && <p className="text-sm text-gray-600 mt-1 max-w-2xl mx-auto">{s.description}</p>}</div>))}</div></section>)}
      {industryExpertise && industryExpertise.length > 0 && industryExpertise.some(e => e.name) && (<section className="mb-10"><h2 className="text-center text-xs font-medium tracking-[0.3em] text-gray-400 mb-4">INDUSTRY EXPERTISE</h2><div className="max-w-md mx-auto space-y-2">{industryExpertise.filter(e => e.name).map((e, i) => (<div key={i}><div className="flex justify-between text-sm"><span className="font-medium text-gray-900">{e.name}</span><span className="text-gray-400">{e.level}</span></div><div className="w-full bg-gray-100 rounded-full h-1 mt-1"><div className="bg-gray-800 h-1 rounded-full" style={{ width: e.level === 'Expert' ? '100%' : e.level === 'Advanced' ? '75%' : e.level === 'Intermediate' ? '50%' : '25%' }}></div></div></div>))}</div></section>)}
      {philosophy && philosophy.quote && (<section className="mb-10 text-center"><h2 className="text-xs font-medium tracking-[0.3em] text-gray-400 mb-4">MY LIFE PHILOSOPHY</h2><blockquote className="italic text-gray-600 max-w-xl mx-auto">&ldquo;{philosophy.quote}&rdquo;</blockquote>{philosophy.author && <p className="text-sm text-gray-500 mt-2">— {philosophy.author}</p>}</section>)}
      {books && books.length > 0 && books.some(b => b.title) && (<section className="mb-10"><h2 className="text-center text-xs font-medium tracking-[0.3em] text-gray-400 mb-4">BOOKS</h2><div className="space-y-2 text-center">{books.filter(b => b.title).map((b, i) => (<div key={i}><h3 className="font-semibold text-gray-900">{b.title}</h3>{b.author && <p className="text-sm text-gray-600">{b.author}</p>}</div>))}</div></section>)}
      {socialLinks && socialLinks.length > 0 && socialLinks.some(l => l.network) && (<section className="mb-10"><h2 className="text-center text-xs font-medium tracking-[0.3em] text-gray-400 mb-4">FIND ME ONLINE</h2><div className="flex flex-wrap justify-center gap-4">{socialLinks.filter(l => l.network).map((l, i) => (<div key={i}><span className="font-semibold text-gray-900 text-sm">{l.network}</span>{l.username && <span className="text-sm text-gray-500 ml-1">{l.username}</span>}</div>))}</div></section>)}
      {signature && (<section className="mb-10 text-center pt-4"><p className="text-2xl italic text-gray-700" style={{ fontFamily: 'cursive' }}>{signature}</p></section>)}
