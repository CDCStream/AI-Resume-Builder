    ...
    <div className="resume-page w-[210mm] min-h-[297mm] bg-white shadow-lg print:shadow-none">
      ...
      <div className="p-10">
        ...
        <header className="mb-8 flex items-start gap-6">
          {basics?.image && (
            <div className="w-24 h-24 rounded-full overflow-hidden border-4 border-rose-200">
              <img
                src={basics.image}
        ...
        <div className="grid grid-cols-3 gap-8">
          ...
          <div className="col-span-2 space-y-8">
                    ...
                    <div key={index}>
                      <div className="flex justify-between items-start mb-1">
                        <div>
                          ...
                          <p className="text-rose-500">{job.name}{(job.city || job.country) && <span className="text-rose-400"> · {[job.city, job.country].filter(Boolean).join(", ")}</span>}</p>
                        </div>
            ...
            {internships && internships.length > 0 && (
              <section className="mb-6">
                <h2 className="text-lg font-bold text-gray-900 mb-4 pb-2 border-b-2 border-rose-200">
                  <span className="text-rose-500">●</span> Internships
                </h2>
                    ...
                    <div key={index}>
                      <div className="flex justify-between items-start mb-1">
                        <div>
                          ...
                          <p className="text-rose-500">{intern.company}{(intern.city || intern.country) && <span className="text-rose-400"> · {[intern.city, intern.country].filter(Boolean).join(", ")}</span>}</p>
                        </div>
                        <span className="text-sm text-gray-500">{formatDate(intern.startDate)} — {formatDate(intern.endDate) || "Present"}</span>
                      </div>
                      {intern.summary && (
          ...
          <div className="space-y-6">
            ...
            {courses && courses.length > 0 && (
              <section className="p-4 bg-gray-50 rounded-lg">
                <h2 className="text-sm font-bold text-gray-900 mb-3 pb-1 border-b border-rose-200">
                  <span className="text-rose-500">●</span> Courses
                </h2>
                <div className="space-y-2">
            ...
            {hobbies && hobbies.length > 0 && hobbies.some(h => h.name) && (
              <section className="p-4 bg-gray-50 rounded-lg mb-4">
                <h2 className="text-sm font-bold text-gray-900 mb-3 pb-1 border-b border-rose-200">
                  <span className="text-rose-500">●</span> Hobbies & Interests
                </h2>
                <div className="flex flex-wrap gap-2">
                  {hobbies.filter(h => h.name).map((hobby, index) => (
                    <span key={index} className="text-sm bg-rose-50 text-rose-600 px-3 py-1 rounded-full">{hobby.name}</span>
                  ))}
                </div>
              </section>
            )}
            ...
            {references && references.length > 0 && references.some(r => r.name) && (
              <section className="p-4 bg-gray-50 rounded-lg mb-4">
                <h2 className="text-sm font-bold text-gray-900 mb-3 pb-1 border-b border-rose-200">
                  <span className="text-rose-500">●</span> References
                </h2>
                <div className="space-y-3">
            ...
            {publications && publications.length > 0 && publications.some(p => p.name) && (
              <section className="p-4 bg-gray-50 rounded-lg">
                <h2 className="text-sm font-bold text-gray-900 mb-3 pb-1 border-b border-rose-200">
                  <span className="text-rose-500">●</span> Publications
                </h2>
                <div className="space-y-3">
                ...
                <div className="space-y-3">
                  {projects.filter(p => p.name).map((project, index) => (
                    <div key={index}>
                      ...
                      {project.url && <a href={project.url} className="text-xs text-rose-500 mt-1 block">{project.url}</a>}
                    </div>
                  ))}
                </div>
              </section>
            ...
            {certificates && certificates.length > 0 && certificates.some(c => c.name) && (
              <section className="p-4 bg-gray-50 rounded-lg">
                <h2 className="text-sm font-bold text-gray-900 mb-3 pb-1 border-b border-rose-200">
                  <span className="text-rose-500">●</span> Certifications
                </h2>
                <div className="space-y-3">
            ...
            {volunteer && volunteer.length > 0 && volunteer.some(v => v.organization) && (
              <section className="p-4 bg-gray-50 rounded-lg">
                <h2 className="text-sm font-bold text-gray-900 mb-3 pb-1 border-b border-rose-200">
                  <span className="text-rose-500">●</span> Volunteering
                </h2>
                <div className="space-y-3">
            ...
            {strengths && strengths.length > 0 && strengths.some(s => s.name) && (<section className="p-4 bg-gray-50 rounded-lg"><h2 className="text-sm font-bold text-gray-900 mb-3 pb-1 border-b border-rose-200"><span className="text-rose-500">●</span> Strengths</h2><div className="space-y-3">{strengths.filter(s => s.name).map((s, i) => (<div key={i}><h3 className="font-semibold text-gray-900 text-sm">{s.name}</h3>{s.description && <p className="text-sm text-gray-600 mt-1">{s.description}</p>}</div>))}</div></section>)}
            {industryExpertise && industryExpertise.length > 0 && industryExpertise.some(e => e.name) && (<section className="p-4 bg-gray-50 rounded-lg"><h2 className="text-sm font-bold text-gray-900 mb-3 pb-1 border-b border-rose-200"><span className="text-rose-500">●</span> Industry Expertise</h2><div className="space-y-2">{industryExpertise.filter(e => e.name).map((e, i) => (<div key={i}><div className="flex justify-between text-sm"><span className="font-medium text-gray-900">{e.name}</span><span className="text-gray-500">{e.level}</span></div><div className="w-full bg-gray-200 rounded-full h-1.5 mt-1"><div className="bg-rose-500 h-1.5 rounded-full" style={{ width: e.level === 'Expert' ? '100%' : e.level === 'Advanced' ? '75%' : e.level === 'Intermediate' ? '50%' : '25%' }}></div></div></div>))}</div></section>)}
            {philosophy && philosophy.quote && (<section className="p-4 bg-gray-50 rounded-lg"><h2 className="text-sm font-bold text-gray-900 mb-3 pb-1 border-b border-rose-200"><span className="text-rose-500">●</span> My Life Philosophy</h2><blockquote className="italic text-gray-600 border-l-2 border-rose-300 pl-3">&ldquo;{philosophy.quote}&rdquo;</blockquote>{philosophy.author && <p className="text-sm text-gray-500 mt-2 text-right">— {philosophy.author}</p>}</section>)}
            {books && books.length > 0 && books.some(b => b.title) && (<section className="p-4 bg-gray-50 rounded-lg"><h2 className="text-sm font-bold text-gray-900 mb-3 pb-1 border-b border-rose-200"><span className="text-rose-500">●</span> Books</h2><div className="space-y-2">{books.filter(b => b.title).map((b, i) => (<div key={i}><h3 className="font-semibold text-gray-900 text-sm">{b.title}</h3>{b.author && <p className="text-xs text-gray-500">{b.author}</p>}</div>))}</div></section>)}
            {socialLinks && socialLinks.length > 0 && socialLinks.some(l => l.network) && (<section className="p-4 bg-gray-50 rounded-lg"><h2 className="text-sm font-bold text-gray-900 mb-3 pb-1 border-b border-rose-200"><span className="text-rose-500">●</span> Find Me Online</h2><div className="space-y-2">{socialLinks.filter(l => l.network).map((l, i) => (<div key={i} className="flex items-center gap-2"><span className="font-semibold text-gray-900 text-sm">{l.network}</span>{l.username && <span className="text-sm text-gray-500">{l.username}</span>}</div>))}</div></section>)}
            {signature && (<section className="p-4 text-center"><p className="text-2xl italic text-gray-700" style={{ fontFamily: 'cursive' }}>{signature}</p></section>)}

            {/* Custom Sections */}
            {customSections && customSections.length > 0 && customSections.map((section, index) => (
              section.title && (
                <section key={index} className="p-4 bg-gray-50 rounded-lg">
                  <h2 className="text-sm font-bold text-gray-900 mb-3 pb-1 border-b border-rose-200">
