...
-- Trigger to create profile only after email confirmation
-- Google OAuth: email_confirmed_at is set at INSERT → profile created immediately
-- Email signup: email_confirmed_at is NULL → no profile until confirmation
...
-- Fire on INSERT (catches Google OAuth where email is confirmed instantly)
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;
CREATE TRIGGER on_auth_user_created
  AFTER INSERT ON auth.users
  FOR EACH ROW EXECUTE FUNCTION public.handle_new_user();

-- Fire on UPDATE when email gets confirmed (catches email signup confirmation)
